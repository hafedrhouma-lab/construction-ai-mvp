...
router.get('/:id/export', async (req, res) => {
  try {
    ...
    // Header with FULL IDs
    csv += '# QuickBids AI Extraction Export\n';
    csv += `# Project ID: ${estimate.project_id}\n`;  // Full UUID
    csv += `# Document ID: ${estimate.file_id}\n`;     // Full UUID
    csv += `# File: ${file.original_filename}\n`;
    csv += `# Topics: "${file.topics ? file.topics.join(', ') : 'None'}"\n`;  // Quoted!
    ...
    csv += 'Project ID,Document ID,Page,Line,Topic,Source,Was Edited,';
    ...
    for (const item of lineItems) {
      ...
      csv += [
        estimate.project_id,          // Full UUID
        estimate.file_id,              // Full UUID
        item.page_number,
        item.line_number,
        `"${item.topic || file.topics?.join(', ') || ''}"`,  // Quoted topics!
        item.source,
...
/**
 * Helper: Format value for CSV (handle NULL)
 */
